...
const defaultHero = {
  d: 'https://images.unsplash.com/photo-1471286174890-9c112cbcd5b8?q=80&w=2000',
    ...
    fetch(`${BASE_URL}/web/storefront${isPreview ? '?preview=true' : ''}`)
      ...
      .then(data => {
        if (data.exito && data.banners) {
          ...
          setHeroImg({
            d: getImgD(b.nino, defaultHero.d),
            m: getImgM(b.nino, defaultHero.m)
          });
        }
      }).catch(console.error);

    // 2. CARGAMOS EL CATÁLOGO COMPLETO DE NIÑO
    fetch(`${BASE_URL}/web/catalogo?genero=Niño`)
      .then(res => res.json())
      .then(data => {
        if (data.exito) {
          const mapeados = data.productos.map((p: any) => {
            ...
            return {
              ...
              imagenes: allImgs,
              tipo: p.tipo || 'Ropa', // Extraemos el tipo de la BD (Playera, Jeans, etc)
