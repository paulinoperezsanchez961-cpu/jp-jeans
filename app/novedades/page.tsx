...
// 1. BASE DE DATOS MAESTRA DE NOVEDADES (Actualizada con arreglo de imágenes)
const productosDb = [
  ...
  { id: 'CHM-05', nombre: 'Puffer Cropped Nocturna', precio: 3499, genero: 'Mujer', fecha: '2026-04-04',
    imagenes: [
...
export default function NovedadesPage() {
  ...
  // EL ALGORITMO DE ORDENAMIENTO (De más nuevo a más viejo)
  ...
  return (
    <div className="bg-white min-h-screen w-full flex flex-col font-sans">
      ...
      {/* BARRA DE FILTROS STICKY (Ajustado z-index a 40) */}
      ...
      {/* CUADRÍCULA DE PRODUCTOS (Fondo blanco con CUADRÍCULA NEGRA de 1px) */}
              ...
              productosMostrar.map((prod) => (
                <div key={prod.id} className="group bg-white flex flex-col relative">
                  ...
                  <Link href={`/producto/${prod.id}`} className="w-full text-left px-3 py-4 flex flex-col">
                    ...
                    <h3 className="text-black font-medium text-[11px] md:text-xs tracking-wide uppercase mb-1">
                      {prod.nombre}
                    </h3>
