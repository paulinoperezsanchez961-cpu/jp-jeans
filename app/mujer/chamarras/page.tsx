...
export default function ChamarrasMujerPage() {
  ...
  return (
    <div className="bg-white min-h-screen w-full flex flex-col font-sans">
            ...
            {menuAbierto === 'talla' && (
              <div className="flex space-x-8">
                {['CH', 'M', 'G'].map(t => (
                  <button 
                    key={t} 
                    onClick={() => { setFiltroTalla(t); setMenuAbierto(null); }} 
            ...
            {menuAbierto === 'tono' && (
              <div className="flex space-x-8">
                {['Claro', 'Oscuro'].map(tono => (
                  <button 
                    key={tono} 
                    onClick={() => { setFiltroTono(tono); setMenuAbierto(null); }} 
      ...
      {/* CUADRÍCULA DE PRODUCTOS (Fondo blanco y cuadrícula negra homogénea) */}
