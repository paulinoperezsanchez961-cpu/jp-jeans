...
// Banners de respaldo por si el servidor tarda o no hay banners subidos en la Oficina
const defaultCortes = [
...
export default function JeansHombrePage() {
  ...
  useEffect(() => {
    ...
    // 1. CARGAMOS LOS BANNERS DE CORTES DESDE LA OFICINA
    fetch(`${BASE_URL}/web/storefront${isPreview ? '?preview=true' : ''}`)
      ...
      .then(data => {
        if (data.exito && data.banners) {
          ...
          setCortes([
            { id: 'baggy', label: 'Baggy', img: getImg(b.h_corte_baggy?.d, defaultCortes[0].img) },
            { id: 'cargo', label: 'Cargo', img: getImg(b.h_corte_cargo?.d, defaultCortes[1].img) },
            { id: 'recto', label: 'Recto', img: getImg(b.h_corte_recto?.d, defaultCortes[2].img) },
            { id: 'slim', label: 'Slim', img: getImg(b.h_corte_slim?.d, defaultCortes[3].img) }
          ]);
        }
      }).catch(console.error);

    // 2. CARGAMOS LOS PRODUCTOS REALES (Hombre + Jeans) Y EXTRAEMOS TALLAS Y DESCRIPCIÓN
    fetch(`${BASE_URL}/web/catalogo?genero=Hombre&tipo=Jeans`)
      .then(res => res.json())
      .then(data => {
        if (data.exito) {
          const mapeados = data.productos.map((p: any) => {
            ...
            const allImgs = [p.url_foto_principal, ...fotosExtra].filter(Boolean).map(img => getImg(img));
            ...
            return {
              ...
              imagenes: allImgs,
              // 💡 AQUI LEEMOS LAS TALLAS REALES DEL CEREBRO
              tallas: p.tallas_array && p.tallas_array.length > 0 ? p.tallas_array : ['28', '30', '32', '34', '36'],
              descripcion: p.descripcion || 'Corte premium de la colección JP Jeans.',
              color: 'Azul' // Podrías hacerlo dinámico si agregas columna de color en la BD
            };
          });
  ...
  return (
    <div className="bg-white min-h-screen w-full flex flex-col font-sans text-black">
      ...
      {/* ESPACIO DEL ENCABEZADO PARA QUE NO TAPE EL CONTENIDO */}
      <div className="w-full h-16 md:h-20 bg-black shrink-0" />
      <div className="w-full h-0.5 bg-white shrink-0" />

      {/* 📐 BANNERS DE CORTES: PROPORCIÓN EXACTA 9:16 */}
      <section className="w-full bg-white relative">
        <Swiper
          modules={[FreeMode]}
          freeMode={true}
          spaceBetween={2}
          slidesPerView={2.2} // 📱 MÓVIL: Muestra 2 completos y un pedacito del tercero para invitar a deslizar
          breakpoints={{
            768: {
              slidesPerView: 4, // 💻 PC: Muestra los 4 exactos
            },
          }}
        ...
        >
          {cortes.map((corte) => (
            <SwiperSlide key={corte.id}>
              ...
              >
                {/* 🚨 ETIQUETA <img /> PARA RESPETAR EL ASPECT RATIO SIN RECORTES */}
                <motion.img 
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
